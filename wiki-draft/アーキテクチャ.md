...
テンプレート待機では、連続して 5 秒見つからないたびにゲームのクライアント領域を 2 秒間観察します。変化したピクセルが 50% を超える場合は戦闘アニメーションの可能性が高いとみなし、その回のリカバリだけをスキップします。それ以外は最後の安全な自動操作位置を 1 回クリックして復旧を試みます。その後、明示的に宣言された次ステップのテンプレートを先に確認し、存在しない場合は現在のステップを冗長に確認してクリックします。成功、タイムアウト、キャンセルのいずれかまで 5 秒周期で待機を続けます。
