...
Link Raid フロー：ナビゲーション、backup requests、更新、参加済み戦闘の整理、レベル選択、参加/LP 処理、戦闘完了、いいね、復帰。レベル選択では、まず選択テンプレートの最良候補を特定し、その同じ位置付近ですべての対応レベルを比較するとともに、中央のレベル数字領域も独立して比較します。全体画像と数字領域の両方が選択レベルと判定した場合だけクリックします。このため全レベルテンプレートが起動時必須の競合グループです。通常のリザルトと参加済み戦闘の整理では、`tap_to_countinue` のクリック成功後にのみ共通のいいね処理を呼び出し、`joined_battles` のクリック自体ではいいね処理を開始しません。LV4 と LV6-LV12 に対応します。選択したレベルが存在しない、またはクリックできない場合は、別のレベルに参加せず更新して再検索します。初期ナビゲーションとスクロールはスケーリングされた 2K 基準座標を使用します。
