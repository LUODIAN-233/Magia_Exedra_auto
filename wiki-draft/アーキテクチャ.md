...
ウィンドウ検索/フォーカス、可視ゲームウィンドウのキャプチャ、OpenCV マッチング、クライアント領域のみの動体サンプリング、マウス操作を担当します。マッチング前にスクリーンショットとテンプレートの双方へ同じ 3x3 Gaussian Blur を適用し、`TM_SQDIFF_NORMED` を使用します。テンプレート認識と復旧用動体検出はゲームのクライアント領域だけを対象にします。ゲームは可視状態で遮蔽されないまま維持する必要があります。
...
自動周回モード、ログレベル、ログ保持日数をルートの `settings.json` に原子的に保存します。
