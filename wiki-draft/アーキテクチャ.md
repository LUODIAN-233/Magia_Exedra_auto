...
Link Raid フロー：ナビゲーション、backup requests、更新、参加済み戦闘の整理、レベル選択、参加/LP 処理、戦闘完了、いいね、復帰。選択候補では全対応レベルと中央数字領域を比較します。9/10、10/10、または人数を確実に読めない候補はスキップし、満員/終了済みの参加結果は閉じて再検索します。`tap_to_countinue` は `_1/2` の文字輪郭だけで結果画面を確認し、スケーリングした画面下中央の安全座標をクリックします。画面が残る場合は再確認後にもう1回までクリックし、`back` の表示後にいいね処理を開始します。いいね一覧のスクロール後はポインターを退避し、短時間再認識します。LV4 と LV6-LV12 に対応し、別レベルへ代替参加せず、ナビゲーションとスクロールには 2K 基準座標を使用します。
