...
Link Raid 流程：导航、求援请求、刷新、已加入战斗清理、等级选择、加入/LP 处理、战斗完成、点赞和返回。正常结算与已加入战斗清理仅在 `tap_to_countinue` 点击成功后调用共用点赞流程；点击 `joined_battles` 本身不会触发点赞。支持 LV4 与 LV6-LV12；若选中等级缺失或无法点击，会刷新并重新搜索，绝不退而加入其它等级。初始导航和滚动使用缩放的 2K 基准坐标。
