...
每次等待模板时，每连续未命中 5 秒就观察游戏客户区 2 秒。变化像素超过 50% 时视为战斗动画，仅跳过本轮恢复；画面稳定时先重新检查下一步，仍未推进才在最后一个安全自动化位置执行恢复点击。对于明确声明了下一步模板的流程，正常点击或恢复点击后都必须先识别到下一步才返回成功；普通模板在当前页面仍存在时可冗余点击，固定安全坐标动作不会立即重复。之后继续按 5 秒周期等待，直到成功、超时或取消。
...
Link Raid 流程：导航、求援请求、刷新、已加入战斗清理、等级选择、加入/LP 处理、战斗完成、点赞和返回。等级选择先定位所选模板的最佳候选，再在同一位置附近比较全部支持等级，并独立比较中央等级数字区域；只有整图和数字区域都将所选等级判为最高时才点击。参加人数为 9/10、10/10 或无法确认的候选会被跳过，加入时的满员/已结束提示会关闭后重新匹配。`tap_to_countinue` 只使用 `_1/2` 的文字前景确认结算页，再点击缩放的底部中央安全坐标；进入 `back` 后才调用共用点赞流程。支持 LV4 与 LV6-LV12；若选中等级缺失或无法点击，会刷新并重新搜索，绝不退而加入其它等级。初始导航和滚动使用缩放的 2K 基准坐标。
...
窗口查找/聚焦、可见游戏窗口截图、OpenCV 匹配、客户区动态采样和鼠标动作。模板读取、Gaussian Blur 与文字前景掩码按文件签名缓存；同一截图可供多组模板复用。普通匹配对截图与模板应用相同的 3x3 Gaussian Blur；文字模式通过 Otsu 前景掩码让 `TM_SQDIFF_NORMED` 忽略动态背景。模板识图和动态恢复检测只采样游戏客户区。游戏必须保持可见且不被遮挡。
...
将挂机模式、日志等级和日志保留天数原子写入根目录 `settings.json`。
