...
- `src/click/click_action.py` - numbered-template iteration, raw/scaled coordinate actions, and tolerant client-resolution detection.
- `src/click/click_behavior.py` - window lookup/focus, visible desktop-region screenshot, OpenCV matching, and mouse action. The game must remain visible and unobstructed.
...
- The game must remain visible and unobstructed. Recognition captures the visible desktop region, repeatedly restores/activates `MadokaExedra`, and uses the global mouse.
...
- **Link Raid:** start on the main/Lighthouse screen. Initial clicks use scaled 2K baselines `(2000,1000)` and `(2400,1200)`. Level range is LV6-LV12.
