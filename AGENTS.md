...
- `src/log_setup.py` - stdlib-only logging configuration called once at `main.py` import, before `QApplication`. When stderr exists, the console handler defaults to WARNING and `MAGIA_LOG_LEVEL` overrides it; windowed builds skip the missing console safely. A rotating DEBUG file handler writes to `logs/`. `main.py` adds a GUI logging handler whose selector is persisted in root `settings.json` via `src/app_settings.py`, defaulting safely to WARNING. Worker `signal.emit` calls remain the unfiltered user-facing GUI log channel and are not replaced by `logging`; runtime `print()` calls have been migrated to `logging.getLogger(__name__)`.
