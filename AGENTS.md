...
- `main.py` (~1100 lines) — entry point. `mywindow(QWidget)` (Qt GUI, title `圆哆啦挂机器`, icon `./resource/main.ico`, `resize(250,740)`) + two mutually exclusive `QThread` workers. Stop is synchronous in the GUI thread and sets both the legacy numeric flag and a `threading.Event`; ordinary UI waits time out after 60 seconds and battle waits after 30 minutes. `LanguageSwitcherWidget(QWidget)` is the language/resolution switcher UI: dropdown labels are Chinese (英语/日语 via `LANG_LABELS`), item *data* holds the real code (`EN`/`JP`); it **auto-switches on selection** (`QComboBox.activated`) — no apply button; the only button is `刷新列表`, which re-scans packs and runs `image_scaler.scale_all()` in a background `threading.Thread`, piping progress to the log via signals. Switching/scaling and automation are mutually exclusive.
...
- `click_behavior.py` — low-level. `get_xy` captures only the `MadokaExedra` window in memory, OpenCV `cv2.matchTemplate(..., cv2.TM_SQDIFF_NORMED)`, `match_rate = 1 - sqrt(min_val)`, threshold **>0.8** hardcoded in `routine`/`routine_only_find`. Missing windows, invalid screenshots/templates, and oversized templates return a normal no-match result instead of raising. `click_auto` does move-then-click (a raw click is rejected by the game), and `routine` rechecks the worker stop callback immediately before clicking.
...
- `image_scaler.py` — pure-stdlib (no Qt) module that scales the `*_2560x1440` source pack to other resolutions using `tools/ImageMagick/magick.exe` (or `magick` on PATH). It keeps `.source_hashes.json` in each derived pack: unchanged SHA-256 sources are skipped, changed files are atomically regenerated, and previously managed files deleted from the source are removed. Incomplete source scans skip cleanup. Never writes into the `*_2560x1440` source pack and never follows junctions/reparse points. Runnable standalone for debugging.
...
- **Run flags are `2`/`1` (and `3` = idle):** `guaji_1` (link raid) and `guaji_2` (crystalis) are module globals. Initial value `3` (not yet started); the GUI sets `1` before starting a worker and `2` when stopping. Each worker also checks its `stop_event_*` to close start/stop races.
...
- **GUI → worker parameters still use globals.** `link_raid_lv_choice` (default `6`, range lv6–lv12), `link_raid_lp_recover_times`, and `crystalis_lp_recover_times` are set from the GUI and copied by workers when they start. Run/stop additionally uses thread events.
...
- `screenshot/` remains as a committed placeholder for compatibility/debugging, but normal matching no longer writes `123.png`; captures stay in memory and are restricted to the game window.
