...
- `src/click/click_action.py` - collects contiguous numbered templates, compares the full group against one shared frame, and chooses only the highest-scoring candidate. Each of the three samples at 0/300/600 ms independently compares the complete group; all winners must exceed their own threshold and remain near the first center, and the click uses their median center. Threshold resolution reuses the worker's locked `expected_pack`. It also performs raw/scaled coordinate actions and detects client resolution tolerantly.
- `src/click/click_behavior.py` - window lookup/focus, client-only screenshot and optional normalized search-region cropping, shared-frame OpenCV matching, client-only motion sampling, and mouse action. Window borders and the title bar are never included in template matching. Matching applies the same 3x3 Gaussian blur to screenshot and templates before `TM_SQDIFF_NORMED`. Recovery positions are stored relative to the client and rebased when the window moves; a client-size change invalidates the saved position. The game must remain visible and unobstructed.
...
- Template clicks require three accepted samples at 0/300/600 ms, and every sample compares the complete template group. Every winning image must pass its own threshold and every center must stay within `max(6px, client_short_side*0.8%)` of the first center; confirmation moves no mouse and the final click uses the median center. Find-only checks, explicit coordinate actions, and last-safe-position recovery clicks do not use this filter.
