...
- `click_action.py` (~94 lines) — high-level actions. `click_item_with_result` / `find_item_with_result` iterate the numbered template group; `click_position(x,y)` and `move_a_to_b(...)` do window-relative coordinate clicks/drags (resolves the window origin via `click_behavior.find_win('MadokaExedra')`). Depends on `click_behavior`.
...
- `image_scaler.py` (~207 lines) — pure-stdlib (no Qt) module that scales the `*_1280x720` source pack up to other resolutions using `tools/ImageMagick/magick.exe`. `scale_factor(res)` returns `1.5`/`2`/`3` or `None` (1.5x→1920x1080, 2x→2560x1440, 3x→3840x2160; non-16:9 or ≤1x skipped). `scale_all(progress_cb)` walks each language's 720p pack and writes scaled copies into the higher-res packs, preserving subfolder structure; existing target files are skipped (delete a target to force regeneration). Never writes into a `*_1280x720` source pack and never follows junctions/reparse points. Runnable standalone for debugging.

Flow: `main.py` workers → `click_action` (template iteration + coords) → `click_behavior` (match + click + window). Separately, `main.py` 刷新列表 → `image_scaler.scale_all` (regenerate higher-res packs from 720p).
...
- Real packs live at `language/<LANG>/<LANG>_<WxH>/` (e.g. `language/EN/EN_1280x720`). The junction target is always one of these; pack folders are never moved or renamed.
- Both source packs are populated: `language/EN/EN_1280x720` and `language/JP/JP_1280x720` each contain the full EN/JP template tree (`crystalis/` + `quests/link_raid/...`). The higher-res packs (`*_1920x1080`, `*_2560x1440`, `*_3840x2160`) start **empty** (`.gitkeep` only) and are filled by 刷新列表.
...
- **Do NOT** manually rename/move `aim/` or merge pack contents — it is a managed junction. To add a template variant, edit the `*_1280x720` source pack; higher-res packs are *derived* by `image_scaler.py` on 刷新列表. To force a stale higher-res file to regenerate, delete it. The 1.5x `*_1920x1080` pack is resampled and slightly less stable than integer 2x/3x packs.
...
- Therefore every empty resolution pack (`*_1920x1080`, `*_2560x1440`, `*_3840x2160`) is kept alive by a committed **`.gitkeep`** so git preserves the directory. Without them, a fresh clone has no derived-pack dirs → the GUI dropdown omits those resolutions and `image_scaler` can't populate them.
...
- Game must run **1280x720 windowed** with the matching template language; the game window must be foreground (the bot can be behind other windows, but the game cannot). The bot forces the game to foreground on start. The JP server can be switched to EN in-game to reuse the EN templates instead of re-capturing.
...
- **Link raid** (`WorkThread_1`): start at the game's main/quest (灯台) screen. The first navigation step (`into_link_raid`) uses **hardcoded window-relative coordinates** (`click_position(1000,500)` then `click_position(1200,600)`), NOT image recognition — only works at 1280x720; switching resolutions breaks the first click. Level range lv6–lv12 (default lv6).
...
- All three files carry the same language switcher as their first line, centered at the top:
  ```
  [简体中文](./README.md) · [English](./README_EN.md) · [日本語](./README_JP.md)
  ```
  Keep it identical across the three files and point links at `./README*.md` (repo-relative).
- **Do not translate:**
