...
- The default match threshold is strictly `> 0.8` via `MATCH_THRESHOLD`; a score equal to 0.8 is not a match. A flow may require a stricter threshold and search region when logs establish a safe domain constraint; Crystalis `result` uses `> 0.85` in the client area's right half.
- Template clicks require three accepted samples at 0/300/600 ms. Every center must stay within `max(6px, client_short_side*0.8%)` of the first center; confirmation moves no mouse and the final click uses the median center. Find-only checks, explicit coordinate actions, and last-safe-position recovery clicks do not use this filter.
...
- **Link Raid:** start on the main/Lighthouse screen. Initial clicks use scaled 2K baselines `(2000,1000)` and `(2400,1200)`. Level choices are LV4 and LV6-LV12; all supported level templates are validated because they compete at the selected candidate position. Missing/unclickable selected levels trigger refresh-and-search, never fallback joining another level.
