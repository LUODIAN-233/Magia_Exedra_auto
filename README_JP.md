...
- クリック可能なテンプレートの待機中は、5秒間認識できないたびにゲームのクライアント領域だけを2秒間観察します。ゲーム画素の50%を超える変化があれば戦闘中としてその回の復旧クリックを行わず、それ以外はbotの直前操作位置を1回クリックします。その後、宣言済みの次ステップが表示されたか確認し、表示されていなければ現在のステップを冗長に確認してクリックしてから、5秒周期の監視を継続します
